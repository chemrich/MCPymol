...
- **A negative residue number was read as a range.** `resi -3` means 1-3 to PyMOL, not residue minus three, so one residue's value was written across three. Negative auth numbering is routine in NMR and EM entries, where it marks expression-tag remnants. Reported against `qscore.py`; `ensembles.py` had it identically and is fixed with it.
- Both are now impossible to reintroduce at a call site: `residue_selection` and `residue_clause` in `mcpymol.wiggles.atoms` are the only places that name a residue, and they quote. A quote *inside* an identifier is refused rather than guessed at, since nothing in the PDB or mmCIF grammar puts one there.
